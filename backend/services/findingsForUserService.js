...
/**
 * Build the same findings list used by GET /api/findings, for chat context.
 * @param {import('mongoose').Types.ObjectId|string} userId
 * @returns {Promise<object[]>}
 */
async function buildFindingsForUser(userId) {
  ...
  for (const buildDomainFindings of DOMAIN_FINDINGS_BUILDERS) {
    const domainFindings = await buildDomainFindings(userId);
  ...
  return sortFindingsDeterministic(findings);
}

/** Thin list for LLM / rule answers (top N by severity). */
function toChatFindingsSummary(findings, limit = 5) {
