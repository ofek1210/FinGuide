...
describe('Findings contribution rate gap integration', () => {
  ...
  it('returns study_fund_rate_below_minimum when implied rate is low', async () => {
    const { token, userId } = await registerAndGetAuth();

    await Document.create({
      user: userId,
      originalName: 'study-low.pdf',
      filename: `study-low-${Date.now()}.pdf`,
      filePath: '/tmp/study-low.pdf',
      fileSize: 1024,
      mimeType: 'application/pdf',
      status: 'completed',
    ...
    await Document.create({
      user: userId,
      originalName: 'study-ok.pdf',
      filename: `study-ok-${Date.now()}.pdf`,
      filePath: '/tmp/study-ok.pdf',
      fileSize: 1024,
      mimeType: 'application/pdf',
      status: 'completed',
    ...
    const ids = (res.body.data || []).map(item => item.id);
    expect(ids).toContain('study_fund_rate_below_minimum');
    const finding = (res.body.data || []).find(item => item.id === 'study_fund_rate_below_minimum');
    expect(finding?.meta?.findingKind).toBe('rate');
    expect(Array.isArray(finding?.meta?.periods)).toBe(true);
  });
});
