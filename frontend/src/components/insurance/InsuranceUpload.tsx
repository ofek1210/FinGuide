/**
 * InsuranceUpload — step 2/2 of the import flow: drop / pick the Har HaBituach
 * Excel report. Real dropzone with drag highlight + states (idle → uploading →
 * parsing → success / error) and a "what happens next" checklist.
 *
 * Wired to the REAL backend — no demo timers. The dropzone calls `onUpload`
 * (→ uploadInsuranceExcel) and derives its state from the live flow state:
 *   - uploading            → busy (progress steps from the server pipeline)
 *   - uploadMsg.type==="error"   → error (real message)
 *   - uploadMsg.type==="success" → success (real imported count)
 * Success is sticky: the user advances with "צפה בתובנות הסוכן" (→ onContinue).
 * Insurance accent = peach; success = mint; validation error = soft red.
 */
import { useEffect, useRef, useState } from "react";
import { AlertTriangle, ArrowLeft, Check, FileSpreadsheet, FileText, Sparkles, Upload } from "lucide-react";
...
export default function InsuranceUpload({
  ...
  useEffect(() => {
    if (document.getElementById("iu-anim")) return;
    const st = document.createElement("style");
    st.id = "iu-anim";
    st.textContent =
      "@keyframes iuRise{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}" +
      "@keyframes iuPop{0%{transform:scale(.5);opacity:0}60%{transform:scale(1.16)}100%{transform:scale(1);opacity:1}}" +
      "@keyframes iuSpin{to{transform:rotate(360deg)}}" +
      "@keyframes iuShake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-4px)}40%,80%{transform:translateX(4px)}}" +
      "@keyframes iuFlow{from{background-position:200% 0}to{background-position:-200% 0}}" +
      "@media (prefers-reduced-motion:reduce){.iu-spin{animation:none!important}.iu-bar{animation:none!important}}";
    document.head.appendChild(st);
  }, []);

  // derive the visual phase from the live flow state
  ...
  // stepped progress feel driven by the real server pipeline steps
  ...
  return (
    <main style={{ maxWidth: 760, margin: "0 auto", padding: "36px 24px 88px" }}>
      ...
      {/* header */}
      <div style={{ marginBottom: 26, animation: "iuRise .5s var(--ease) both" }}>
        ...
        <h1 style={{ margin: 0, fontSize: "clamp(27px,3.4vw,38px)", fontWeight: 900, letterSpacing: "-.035em", lineHeight: 1.06, color: "var(--text-strong)" }}>העלה את קובץ הביטוח שלך</h1>
      ...
      {/* upload panel */}
      <div style={{ background: "var(--card)", border: "1px solid var(--border-hair)", borderRadius: "var(--radius)", boxShadow: "var(--shadow-card)", padding: 22, animation: "iuRise .5s var(--ease) .06s both" }}>
        {/* dropzone */}
        <div
          ...
          style={{
            ...
            border: `2px dashed ${phase === "drag" ? "var(--peach-ink)" : phase === "error" ? "rgba(214,69,69,.5)" : "var(--border-soft)"}`,
            background: phase === "drag" ? "var(--peach-soft)" : "var(--surface-sunken)",
            backgroundImage: "radial-gradient(rgba(123,95,214,.06) 1px,transparent 1px)", backgroundSize: "16px 16px",
        ...
        >
          ...
          <div style={{ width: 64, height: 64, margin: "0 auto 18px", borderRadius: 16, display: "grid", placeItems: "center",
            ...
            color: phase === "success" ? "var(--mint-ink)" : phase === "error" ? "#C23B3B" : "var(--mint-ink)",
            boxShadow: "var(--shadow-soft)", border: "1px solid var(--border-hair)" }}>
            {phase === "busy" ? (
              <span className="iu-spin" style={{ width: 28, height: 28, borderRadius: "50%", border: "3px solid var(--lav-100)", borderTopColor: "var(--lav-600)", animation: "iuSpin .8s linear infinite", display: "block" }} />
            ) : phase === "success" ? (
              <span style={{ display: "grid", placeItems: "center", animation: "iuPop .4s var(--ease)" }}><Check size={34} strokeWidth={2.6} /></span>
            ) : phase === "error" ? (
          ...
          {phase === "idle" || phase === "drag" ? (
            <>
              <div style={{ fontSize: 19, fontWeight: 900, letterSpacing: "-.02em", marginBottom: 6, color: "var(--text-strong)" }}>גרור את קובץ ה‑Excel לכאן</div>
              <div style={{ fontSize: 14, color: "var(--text-muted)", fontWeight: 500, marginBottom: 18 }}>או לחץ לבחירה מהמחשב</div>
              <button
                onClick={e => { e.stopPropagation(); pick(); }}
                style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "13px 26px", borderRadius: "var(--r-md)", border: "none", cursor: "pointer", fontFamily: "var(--font-body)", fontWeight: 800, fontSize: 15, color: "#fff", background: "var(--lav-600)", boxShadow: "var(--shadow-soft)", transition: "filter .2s" }}
                onMouseEnter={e => { e.currentTarget.style.filter = "brightness(1.07)"; }}
                onMouseLeave={e => { e.currentTarget.style.filter = "none"; }}
              >
                <FileText size={17} /> בחר קובץ xlsx
              </button>
              <div style={{ fontSize: 12.5, color: "var(--text-faint)", fontWeight: 600, marginTop: 16 }}>פורמטים נתמכים: .xls, .xlsx · עד 5MB</div>
            </>
          ) : phase === "busy" ? (
            <>
              ...
              <div style={{ maxWidth: 340, margin: "0 auto", height: 8, borderRadius: 999, background: "var(--hair)", overflow: "hidden" }}>
                <div className="iu-bar" style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg,var(--lav-600),var(--mint-ink),var(--lav-600))", backgroundSize: "200% 100%", borderRadius: 999, transition: "width .5s var(--ease)", animation: "iuFlow 1.4s linear infinite" }} />
              </div>
              <div style={{ fontSize: 12.5, color: "var(--text-faint)", fontWeight: 700, marginTop: 10 }}>כמעט שם — מזהה פוליסות</div>
            </>
          ) : phase === "success" ? (
            <>
            ...
            <>
              ...
              <div style={{ fontSize: 13.5, color: "var(--text-muted)", fontWeight: 500, marginBottom: 18, maxWidth: 360, marginInline: "auto", lineHeight: 1.5 }}>ודא שזהו קובץ ה‑Excel המקורי שהורד מהר הביטוח, בפורמט .xlsx.</div>
              <button
                onClick={e => { e.stopPropagation(); pick(); }}
        ...
        {/* error banner (mirrors the real validation message) */}
        {phase === "error" && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 14, padding: "13px 16px", borderRadius: "var(--r-sm)", background: "rgba(214,69,69,.07)", border: "1px solid rgba(214,69,69,.22)", animation: "iuShake .4s var(--ease)" }}>
            <span style={{ color: "#C23B3B", flex: "none", display: "inline-flex" }}><AlertTriangle size={17} /></span>
            <span style={{ fontSize: 13.5, fontWeight: 700, color: "#A93232" }}>{uploadMsg?.text ?? "לא הצלחנו לפרסר את הקובץ. ודא שזהו קובץ הר הביטוח תקין."}</span>
          </div>
        )}

        {/* continue (only once analyzed) */}
        {phase === "success" && (
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16, animation: "iuRise .4s var(--ease)" }}>
            <button
              onClick={onContinue}
            ...
            >
              <ArrowLeft size={17} style={{ transform: "scaleX(-1)" }} /> צפה בתובנות הסוכן
            </button>
          </div>
        )}
      </div>
      ...
      {/* what happens next */}
      <div style={{ marginTop: 22, padding: "22px 24px", borderRadius: "var(--radius)", background: "var(--lav-50)", border: "1px solid var(--border-soft)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 800, color: "var(--lav-600)", marginBottom: 16 }}>
